...
public class CDAdEvents {
    ...
    static EventDispatcher getDispatcher() {
        ...
        if (result == null) {
            synchronized (CDAdEvents.class) {
                result = sEventDispatcher;
                if (result == null) {
                    ArrayList<EventRecorder> recorders = new ArrayList<EventRecorder>();
                    HandlerThread handlerThread = new HandlerThread("chalkdigital_event_logging");
                    handlerThread.start();
