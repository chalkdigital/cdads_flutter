 ...
 */
public class CDAdLifecycleManager implements LifecycleListener {
    private static CDAdLifecycleManager sInstance;
    ...
    private CDAdLifecycleManager(Activity mainActivity) {
        mLifecycleListeners = new HashSet<LifecycleListener>();
        mMainActivity = new WeakReference<Activity>(mainActivity);
    }

    @NonNull public static synchronized CDAdLifecycleManager getInstance(Activity mainActivity) {
        if (sInstance == null) {
            sInstance = new CDAdLifecycleManager(mainActivity);
        }
