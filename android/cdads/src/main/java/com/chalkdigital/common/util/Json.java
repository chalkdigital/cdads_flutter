...
public class Json {
    ...
    public static <T> T getJsonValue(final JSONObject jsonObject, final String key, final Class<T> valueClass) {
        ...
        if (object == null) {
            CDAdLog.w("Tried to get Json value with key: " + key + ", but it was null");
            return null;
        } else if (!valueClass.isInstance(object)) {
            CDAdLog.w("Tried to get Json value with key: " + key + ", of type: " + valueClass.toString() + ", its type did not match");
            return null;
        }
